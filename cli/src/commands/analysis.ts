...
export class AnalysisCommands {
      ...
      console.log(
        JSON.stringify({
          ...
          summary: {
            ...
            complexityScore:
              result.analysis?.qualityMetrics?.complexityScore || 0,
      ...
      console.log(
        JSON.stringify({
          ...
          summary: {
            ...
            improvingMetrics:
              result.qualityAnalysis?.trends?.improving?.length || 0,
            decliningMetrics:
              result.qualityAnalysis?.trends?.declining?.length || 0,
            recommendations:
  ...
  async batch(inputFile: string, options: any): Promise<void> {
    try {
      ...
      if (!Array.isArray(operations)) {
        throw new Error(
          'Input file must contain an array of analysis operations',
        );
      }
  ...
  private findStrongestRelationship(result: any): any {
    ...
    return allRelationships.reduce((strongest: any, current: any) => {
      return (current.strength || 0) > (strongest.strength || 0)
        ? current
        : strongest;
    });
  }
