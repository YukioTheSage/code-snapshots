...
import { validateSnapshotId } from '../utils/pathSecurity';
import {
  assertBoolean,
  assertNumber,
  assertString,
...
function validateSnapshotFileEntry(
  value: unknown,
  fieldName: string,
): void {
  ...
  if ('content' in value && value.content !== undefined && value.content !== null && typeof value.content !== 'string') {
...
export function validateSnapshotIndex(
  ...
  value.snapshots.forEach((item, index) => {
    ...
    assertString(item.description, `snapshotIndex.snapshots[${index}].description`, {
      allowEmpty: true,
    });
  });
